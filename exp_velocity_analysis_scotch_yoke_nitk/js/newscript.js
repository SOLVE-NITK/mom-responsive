...
$( "#crankslider" ).on( "slide", function( e, ui ) { $('#crankspinner').spinner("value",ui.value); j=20;ptxdot=[];ptx=[]; pty=[]; } );
$( "#crankspinner" ).on( "spin", function( e, ui ) { $('#crankslider').slider("value",ui.value); j=20;ptxdot=[];ptx=[]; pty=[]; } );
...
// monitoring change in value and connecting slider and number
// setting trace point coordinate arrays to empty on change of link length
$( "#omegaslider" ).on( "slide", function( e, ui ) { $('#omegaspinner').spinner("value",ui.value); j=20;ptxdot=[]; ptx=[]; pty=[];  } );
$( "#omegaspinner" ).on( "spin", function( e, ui ) { $('#omegaslider').slider("value",ui.value); j=20;ptxdot=[]; ptx=[]; pty=[]; } );
...
printcomment("Simple Harmonic Motion <br> v = - &omega; * A * sin( &omega; t )",1);
...
printcomment("Velocity (mm/s) <br> v = -"+ omega +" &times; "+ l +" &times; sin("+theta+"&deg;)  = "+ roundd(-omega*l*Math.sin(rad(theta)),2),2);
}
