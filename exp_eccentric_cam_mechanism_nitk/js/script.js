...
// monitoring change in value and connecting slider and number
// setting trace point coordinate arrays to empty on change of link length
$( "#aslider" ).on( "slide", function( e, ui ) { $('#aspinner').spinner("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;	ptx.push(b.ycoord-50); pty.push(o.xcoord+j);} );
$( "#aspinner" ).on( "spin", function( e, ui ) { $('#aslider').slider("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;	ptx.push(b.ycoord-50); pty.push(o.xcoord+j);} );
...
// monitoring change in value and connecting slider and number
// setting trace point coordinate arrays to empty on change of link length
$( "#thetaslider" ).on( "slide", function( e, ui ) { $('#thetaspinner').spinner("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; j=20;	ptx.push(b.ycoord-50); pty.push(o.xcoord+j);} );
$( "#thetaspinner" ).on( "spin", function( e, ui ) { $('#thetaslider').slider("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; j=20;	ptx.push(b.ycoord-50); pty.push(o.xcoord+j);} );
...
// monitoring change in value and connecting slider and number
// setting trace point coordinate arrays to empty on change of link length
$( "#omegaslider" ).on( "slide", function( e, ui ) { $('#omegaspinner').spinner("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;	ptx.push(b.ycoord-50); pty.push(o.xcoord+j);} );
$( "#omegaspinner" ).on( "spin", function( e, ui ) { $('#omegaslider').slider("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;	ptx.push(b.ycoord-50); pty.push(o.xcoord+j);} );
...
printcomment("Limits of a for the set r : "+$('#aspinner').spinner("option","min")+" and\n "+$('#aspinner').spinner("option","max")+" ",1);
...
if(document.getElementById("graphPlot").checked==true)
{
  ...
  b1.xcoord=b.xcoord;b1.ycoord=b.ycoord-25;
...
phi = deg(Math.asin((aa*Math.cos(rad(theta)))/r));
vel = aa*omega*(Math.cos(rad(theta))+Math.sin(rad(theta))*Math.tan(rad(phi)));
acc = aa*Math.pow(omega,2)*(-Math.sin(rad(theta))+Math.cos(rad(theta))*Math.tan(rad(phi))-(aa/r)*Math.pow(Math.sin(rad(theta))/Math.cos(rad(phi)),2));
jerk = aa*Math.pow(omega,3)*(-Math.cos(rad(theta))-Math.sin(rad(theta))*Math.tan(rad(phi))-(aa/(2*r))*Math.sin(2*rad(theta))/Math.pow(Math.cos(rad(phi)),3)-(aa/r)*Math.sin(2*rad(theta))/Math.pow(Math.cos(rad(phi)),2)+2*Math.pow(aa/r,2)*Math.pow(Math.sin(rad(theta)),3)*Math.sin(rad(phi))/Math.pow(Math.cos(rad(phi)),4));
