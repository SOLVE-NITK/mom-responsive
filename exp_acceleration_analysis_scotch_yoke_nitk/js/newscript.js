...
$( "#crankslider" ).on( "slide", function( e, ui ) { $('#crankspinner').spinner("value",ui.value); j=20; ptxddot=[]; ptxdot=[];ptx=[]; pty=[]; } );
$( "#crankspinner" ).on( "spin", function( e, ui ) { $('#crankslider').slider("value",ui.value); j=20; ptxddot=[]; ptxdot=[];ptx=[]; pty=[]; } );
...
// monitoring change in value and connecting slider and number
// setting trace point coordinate arrays to empty on change of link length
$( "#omegaslider" ).on( "slide", function( e, ui ) { $('#omegaspinner').spinner("value",ui.value); j=20; ptxddot=[]; ptxdot=[]; ptx=[]; pty=[];  } );
$( "#omegaspinner" ).on( "spin", function( e, ui ) { $('#omegaslider').slider("value",ui.value); j=20; ptxddot=[]; ptxdot=[]; ptx=[]; pty=[]; } );
...
printcomment("Simple Harmonic Motion <br> a = - &omega;<sup>2</sup> * A * cos( &omega; t )",1);
...
printcomment("Acceleration (mm/s<sup>2</sup>) <br> a = -"+ omega +"<sup>2</sup> &times; "+ l +" &times; cos("+theta+"&deg;)</br>= "+ roundd(-omega*omega*l*Math.cos(rad(theta)),2),2);
}
...
if(document.getElementById("graphPlot").checked==true)
   ...
   context.moveTo(o.xcoord+145,o.ycoord);
	...
	for( forvar=0; forvar<=300;forvar+=20) //amplitude axis marking
	{
