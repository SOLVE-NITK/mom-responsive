...
function varupdate() {
  ...
  else if (Math.abs(r * omega2) >= 150 && Math.abs(r * omega2) < 400) scaleV = 0.5;
...
function draw() {
  ...
  ctx.fillStyle = "#000";
  if (scaleV >= 1)
    ctx.fillText("Scale = 1:" + scaleV, 15, 165);
  if (scaleV < 1)
    ctx.fillText("Scale = " + 1 / scaleV + ":1", 15, 165);
  ctx.restore();
  drawvel(ctx);
...
function drawvel(context) {
  ...
  drawArrow(va.xcoord, va.ycoord, ctx, 180 - theta2 - rotstatus * 90, 15, 30, "#FF0000");


  pointjoin(va, vba, ctx, "#00FF00", 2);
  drawArrow(vba.xcoord, vba.ycoord, ctx, 180 + phi + signof(velba) * 90, 15, 30, "#00FF00");

  pointjoin(vo, vb, ctx, "#000000", 2);
  drawArrow(vb.xcoord, vb.ycoord, ctx, 90 - (signof(velb) * 90), 15, 30, "#000000");
...
function drawrem(context) {
  ...
  context.font = "14px 'Comic Sans MS'";
  context.fillStyle = "#000000";
  context.fillText("Velocity Diagram", 15, 150);
  context.restore();
  // r, l, d display
  context.save();
  context.lineWidth = 1;
