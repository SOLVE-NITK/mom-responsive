...
var r1= 50, r2, r3, r4=2*r1 ,r5 = 2*r1,l,ldot,lddot;
var  th3=0, th4=0;
var state = "v";
...
var omega3=0 , omega4=0, v1=0, v2=0, v3=0, v12=0, v23=0, vs=0;
var a1n=0,a1t=0,a2s=0,a2c=0,a3n=0,a3t=0,a34n=0,a34t=0,a4s=0,alph3=0,alph4=0;
var temp = 0;
var state1 = 1
...
$( "#con_rodslider" ).on( "slide", function( e, ui ) { $('#con_rodspinner').spinner("value",ui.value); ptx=[]; pty=[]; } );
$( "#con_rodspinner" ).on( "spin", function( e, ui ) { $('#con_rodslider').slider("value",ui.value); ptx=[]; pty=[]; } );
$( "#con_rodspinner" ).on( "change", function() {  varchange() } );
$( "#ramslider" ).on( "slide", function( e, ui ) { $('#ramspinner').spinner("value",ui.value); ptx=[]; pty=[]; } );
$( "#ramspinner" ).on( "spin", function( e, ui ) { $('#ramslider').slider("value",ui.value); ptx=[]; pty=[]; } );
$( "#ramspinner" ).on( "change", function() {  varchange() } );
// Angle Alpha
// slider initialisation : jQuery widget
...
printcomment("<center>E is the point on link BC whereas A is the point on the slider.<br> <b>Vea</b> : Velocity of E wrt A<br><b> Adc</b> : Acceleration of D wrt C</center>",1);
if(simstatus)
{
...
l = Math.sqrt((a.ycoord-b.ycoord)*(a.ycoord-b.ycoord)+(a.xcoord-b.xcoord)*(a.xcoord-b.xcoord));
ldot = (r1*r4*Math.cos(rad(thi))*omega2)/l;
lddot = (r1*r4)*((- omega2*omega2*Math.sin(rad(thi)) - Math.cos(rad(thi))*omega2)/(l*l));
v1 = omega2*r1;

omega3 = (ldot*Math.cos(rad(th3)) + r1*omega2*Math.sin(rad(thi)))/(l*Math.sin(rad(th3)));//(Math.sin(rad(thi)) + (r4/l)*Math.cos(rad(thi)*Math.cos(rad(thi)))) * ((r1*omega2)/(l*Math.sin(rad(th3))));
//console.log('l',l)
vs = ldot;
v2 = omega3*r2;
//console.log('vc',vc);
...
a1t = 0;
alph3 = (lddot*Math.cos(rad(th3)) - 2*ldot*omega3*Math.sin(rad(th3)) - l*omega3*omega3*Math.cos(rad(th3)) + r1*omega2*omega2*Math.cos(rad(thi)))/(l*Math.sin(rad(th3)));

a3t = r2*alph3;
a2c = 2*ldot*omega3;
a2s = lddot;

alph4 = (r2*alph3*Math.cos(rad(th3)) - r2*omega3*omega3*Math.sin(rad(th3)) + r3*omega3*omega3*Math.sin(rad(th4)))/(r3*Math.cos(rad(th4)));
a4s = -r2*alph3*Math.sin(rad(th3)) - r2*omega3*omega3*Math.cos(rad(th3)) + r3*alph4*Math.sin(rad(th4)) + r3*omega4*omega4*Math.cos(rad(th4));
a34t = r3*alph4;
log();
if (omega2 > 0.8 && omega2<=1.2) 
  {
...
//Velocity Diagram
  ...
  drawArrow(va.xcoord,va.ycoord,ctx,180-thi-rotstatus*90,15,30,"#0066FF");

  pointjoin(c,vc,ctx,"#D00000",2);
  drawArrow(vc.xcoord,vc.ycoord,ctx,180-th3-(signof(omega3))*90,15,30,"#D00000");
...
function printcomment(commenttext,commentloc)
{
  ...
  // ignore use of deprecated tag <center> . Code is executed only if printcomment function receives inappropriate commentloc value
    ...
    function next() {
      ...
      document.getElementById('prev').src="images/bluebkdulls.png";
    ...
    function prev() {
      ...
      document.getElementById('next').src="images/bluefwdulls.png";
      functions[currentFunction]();
      console.log('currentFunction',currentFunction)
      //p();
      count = count+1;
