...
var theta=0, theta2=0;
var omega=0, omega2=0;
//var a=0,b=0;
...
var truncate=750;
var forvar=0;
var m1=0,m2=0,x=0,y=0;
var p=0, q=0, jj=1, flag=1;
//var v1=0 ,v2=0 ,v3=0;
...
$( "#betaslider" ).on( "slide", function( e, ui ) { $('#betaspinner').spinner("value",ui.value);jj=1;theta=0;ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20; ptx.push(30); ptxdot.push(270);ptxddot.push(270-ptx);pty.push(300);omega2=omega*Math.cos(rad(beta))/(1-Math.pow(Math.sin(rad(beta)),2)*Math.pow(Math.cos(rad(theta)),2));ptxdddot.push(150-50*(omega2-omega)); });
$( "#betaspinner" ).on( "spin", function( e, ui ) { $('#betaslider').slider("value",ui.value); jj=1;ptx=[];theta=0; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;ptx.push(30); ptxdot.push(270);ptxddot.push(270-ptx);pty.push(300);omega2=omega*Math.cos(rad(beta))/(1-Math.pow(Math.sin(rad(beta)),2)*Math.pow(Math.cos(rad(theta)),2));ptxdddot.push(150-50*(omega2-omega));} );
$( "#betaspinner" ).on( "change", function() {  varchange() } );
...
// monitoring change in value and connecting slider and number
// setting trace point coordinate arrays to empty on change of link length
$( "#thetaslider" ).on( "slide", function( e, ui ) { $('#thetaspinner').spinner("value",ui.value);ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20; ptx.push(30); ptxdot.push(270);ptxddot.push(270-ptx);pty.push(300);omega2=omega*Math.cos(rad(beta))/(1-Math.pow(Math.sin(rad(beta)),2)*Math.pow(Math.cos(rad(theta)),2));ptxdddot.push(150-50*(omega2-omega));} );
$( "#thetaspinner" ).on( "spin", function( e, ui ) { $('#thetaslider').slider("value",ui.value);ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20; ptx.push(30); ptxdot.push(270);ptxddot.push(270-ptx);pty.push(300);omega2=omega*Math.cos(rad(beta))/(1-Math.pow(Math.sin(rad(beta)),2)*Math.pow(Math.cos(rad(theta)),2));ptxdddot.push(150-50*(omega2-omega));} );
...
// monitoring change in value and connecting slider and number
// setting trace point coordinate arrays to empty on change of link length
$( "#omegaslider" ).on( "slide", function( e, ui ) { $('#omegaspinner').spinner("value",ui.value); jj=1;theta=0;ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;ptx.push(30); ptxdot.push(270);ptxddot.push(270-ptx);pty.push(300);omega2=omega*Math.cos(rad(beta))/(1-Math.pow(Math.sin(rad(beta)),2)*Math.pow(Math.cos(rad(theta)),2));ptxdddot.push(150-50*(omega2-omega));} );
$( "#omegaspinner" ).on( "spin", function( e, ui ) { $('#omegaslider').slider("value",ui.value);jj=1;theta=0; ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;ptx.push(30); ptxdot.push(270);ptxddot.push(270-ptx);pty.push(300);omega2=omega*Math.cos(rad(beta))/(1-Math.pow(Math.sin(rad(beta)),2)*Math.pow(Math.cos(rad(theta)),2));ptxdddot.push(150-50*(omega2-omega));} );
$( "#omegaspinner" ).on( "change", function() {  varchange() } );
...
printcomment("Input Shaft at "+roundd(theta,2)+"&deg; <br> Output Shaft at "+roundd(theta2-90,2)+"&deg;",1);
printcomment("Input Angular Velocity = "+roundd(omega,2)+"rad/s <br>Output Angular Velocity = "+roundd(omega2,3)+"rad/s",2);
...
omega2=omega*Math.cos(rad(beta))/(1-Math.pow(Math.sin(rad(beta)),2)*Math.pow(Math.cos(rad(theta)),2));
ox=o.xcoord=0;
oy=o.ycoord=0;
ax=a.xcoord=0;
...
a1x=a1.xcoord=-a.xcoord; a1y=a1.ycoord=-a.ycoord;
b1x=b1.xcoord=-b.xcoord; b1y=b1.ycoord=-b.ycoord;

u.xcoord=r*Math.cos(rad(theta+90)); 
u.ycoord=r*Math.sin(rad(theta+90));
u1.xcoord=-u.xcoord; u1.ycoord=-u.ycoord;
vx=v.xcoord=r*Math.cos(rad(beta))*Math.cos(rad(theta2+90)); 
vy=v.ycoord=r*Math.cos(rad(beta))*Math.sin(rad(theta2+90));
v1x=v1.xcoord=-v.xcoord; v1y=v1.ycoord=-v.ycoord;
...
ctx.lineWidth=7;
s.xcoord=52*Math.cos(rad(beta)); s.ycoord=-52*Math.sin(rad(beta));
pointtrans(s,trans);
ctx.moveTo(s.xcoord,s.ycoord);
s.xcoord=75*Math.cos(rad(beta)); s.ycoord=-75*Math.sin(rad(beta));
pointtrans(s,trans);
...
if(document.getElementById("graphPlot").checked==true){
pointtrans(u,trans1); pointtrans(u1,trans1);
pointtrans(v,trans1); pointtrans(v1,trans1);
}
if(document.getElementById("graphPlot").checked==false){
pointtrans(u,transo1); pointtrans(u1,transo1);
pointtrans(v,transo1); pointtrans(v1,transo1);
}
...
s.xcoord=o.xcoord+200; s.ycoord=o.ycoord+v1x+23/2*Math.sin(rad(beta));
...
while(i<pty.length)
{	
	if(pty[i]>200 && jj>0){context.lineTo(270,30);break;}
	else
...
while(i<pty.length)
{	
	if(pt[i]<400 && jj>0){context.lineTo(540,pty[i]);break;}
	else
	context.lineTo(pt[i],pty[i]);
	i++;
	if(pt[i]>500)jj++;
