...
$( "#aslider" ).on( "slide", function( e, ui ) { $('#aspinner').spinner("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;	ptx.push(s.ycoord-50); pty.push(o.xcoord+j);});
$( "#aspinner" ).on( "spin", function( e, ui ) { $('#aslider').slider("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;	ptx.push(s.ycoord-50); pty.push(o.xcoord+j);} );
...
// monitoring change in value and connecting slider and number
// setting trace point coordinate arrays to empty on change of link length
$( "#bslider" ).on( "slide", function( e, ui ) { $('#bspinner').spinner("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;	ptx.push(s.ycoord-50); pty.push(o.xcoord+j);} );
$( "#bspinner" ).on( "spin", function( e, ui ) { $('#bslider').slider("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;	ptx.push(s.ycoord-50); pty.push(o.xcoord+j);} );
...
// monitoring change in value and connecting slider and number
// setting trace point coordinate arrays to empty on change of link length
$( "#thetaslider" ).on( "slide", function( e, ui ) { $('#thetaspinner').spinner("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; j=20;	ptx.push(s.ycoord-50); pty.push(o.xcoord+j);} );
$( "#thetaspinner" ).on( "spin", function( e, ui ) { $('#thetaslider').slider("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; j=20;	ptx.push(s.ycoord-50); pty.push(o.xcoord+j);} );
...
// monitoring change in value and connecting slider and number
// setting trace point coordinate arrays to empty on change of link length
$( "#omegaslider" ).on( "slide", function( e, ui ) { $('#omegaspinner').spinner("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;	ptx.push(s.ycoord-50); pty.push(o.xcoord+j);} );
$( "#omegaspinner" ).on( "spin", function( e, ui ) { $('#omegaslider').slider("value",ui.value); ptx=[]; pty=[]; ptxdot=[]; ptxddot=[]; ptxdddot=[]; j=20;	ptx.push(s.ycoord-50); pty.push(o.xcoord+j);} );
...
printcomment("Maj Axis at "+theta+"&deg;  Position = "+roundd(pos,2)+"cm<br>Vel = "+roundd(-vel,2)+"cm/s  Acc = "+roundd(-acc,2)+"cm/s^2<br>Jerk = "+roundd(-jerk,2)+"cm/s^3",2);
}
...
if(document.getElementById("graphPlot").checked==false)
{
 ...
 ctx.clearRect(0,0,550,450);  //clears the complete canvas#simscreen everytime
  ...
  s1.xcoord=s.xcoord; s1.ycoord=s.ycoord-25;
...
pos = a*b*Math.pow((1+m1*m1)/(b*b+a*a*m1*m1),0.5)-(a+b)/2;
vel = a*b*m1*omega*(a*a-b*b)/Math.pow(Math.sin(rad(theta)),2)/Math.sqrt(1+m1*m1)/Math.pow(b*b+a*a*m1*m1,1.5);
...
if(document.getElementById("graphPlot").checked==true)
{
	...
	context.fillText("Position (s)", o.xcoord-47,o.ycoord-(a+b)/2-50);	
	context.fillText("Velocity (v)", o.xcoord-50,o.ycoord-(a+b)/2-125);	
	context.fillText("Acceleration (a)", o.xcoord-75,o.ycoord-(a+b)/2-200);	
	...
	for( forvar=50; forvar<=275;forvar+=75) //amplitude axis marking
	{
...
context.lineWidth=2;
context.moveTo(o.xcoord+a*Math.cos(rad(theta)),o.ycoord-a*Math.sin(rad(theta)));
i=0;
