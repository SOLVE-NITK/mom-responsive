...
$( "#crankslider" ).on( "slide", function( e, ui ) { $('#crankspinner').spinner("value",ui.value); j=20;ptx=[]; pty=[]; } );
$( "#crankspinner" ).on( "spin", function( e, ui ) { $('#crankslider').slider("value",ui.value); j=20;ptx=[]; pty=[]; } );
...
// monitoring change in value and connecting slider and number
// setting trace point coordinate arrays to empty on change of link length
$( "#omegaslider" ).on( "slide", function( e, ui ) { $('#omegaspinner').spinner("value",ui.value); j=20; ptx=[]; pty=[];  } );
$( "#omegaspinner" ).on( "spin", function( e, ui ) { $('#omegaslider').slider("value",ui.value); j=20; ptx=[]; pty=[]; } );
...
printcomment("Displacement (mm) <br> s = "+ l +" &times; cos("+theta+"&deg;)  = "+ roundd(l*Math.cos(rad(theta)),2),2);
}
...
if(document.getElementById("graphPlot").checked==true)
   ...
   context.moveTo(o.xcoord+145,o.ycoord);
	...
	for( forvar=0; forvar<=100;forvar+=20) //amplitude axis marking
	{
