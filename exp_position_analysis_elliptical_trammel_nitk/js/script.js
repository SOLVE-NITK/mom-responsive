...
$( "#ABslider" ).on( "slide", function( e, ui ) { $('#ABspinner').spinner("value",ui.value); ptx=[]; pty=[]; count=0;} );
$( "#ABspinner" ).on( "spin", function( e, ui ) { $('#ABslider').slider("value",ui.value); ptx=[]; pty=[]; count=0; } );
$( "#ABspinner" ).on( "change", function() {  count=0;varchange(); } );
...
// monitoring change in value and connecting slider and number
// setting trace point coordinate arrays to empty on change of link length
$( "#ACslider" ).on( "slide", function( e, ui ) { $('#ACspinner').spinner("value",ui.value); ptx=[]; pty=[]; count=0; } );
$( "#ACspinner" ).on( "spin", function( e, ui ) { $('#ACslider').slider("value",ui.value); ptx=[]; pty=[]; count=0; } );
$( "#ACspinner" ).on( "change", function() { count=0; varchange(); } );
...
function drawdimensions(context)
{
  ...
  drawArrow(trans.xcoord-220,trans.ycoord+lC,context,270,15,30,'','',"#000");
  drawArrow(trans.xcoord-220,trans.ycoord-lC,context,90,15,30,'','',"#000");
  drawArrow(trans.xcoord+Math.abs(lC-l),trans.ycoord+200,context,180,15,30,'','',"#000");
  drawArrow(trans.xcoord-Math.abs(lC-l),trans.ycoord+200,context,0,15,30,'','',"#000");
